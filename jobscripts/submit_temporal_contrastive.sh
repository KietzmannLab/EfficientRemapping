#!/bin/bash
#SBATCH --job-name=temporal_contrastive_RNN
...
#SBATCH --output=logs/temporal_contrastive_%j.out
#SBATCH --error=logs/temporal_contrastive_%j.err
...
python src/train_temporal_contrastive.py \
    ...
    --time_steps_img 6 \
    --time_steps_cords 3 \
    --model_name "temporal_contrastive_global_negatives_t05_nb2_middle" \
