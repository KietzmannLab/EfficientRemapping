...
class MNISTDataset(Dataset):
    ...
    Taken from Ali et al., Adjusted by Thomas Nortmann into a grid of mnist digits with eye movement between grid positions
    ...
    def create_list_batches(self, batch_size, sequence_length, shuffle=True, distractor=False,fixed_starting_point=0):
        data, labels = create_sequence_lists(self, sequence_length, batch_size, shuffle, distractor, fixed_starting_point)
        data = data.repeat_interleave(self.repeat, dim=1)
        labels = labels.repeat_interleave(self.repeat, dim=1)
        return data, labels
...
def create_sequence_lists(dataset, sequence_length, batch_size, shuffle=True, distractor=False, fixed_starting_point=0, shuffle_digit_positions=True):
    # number of datapoints
    data_size = dataset.x.shape[0]
    ...
    sequences = sequences[:cutoff]
    # select the data points and group per sequence and batch
    if shuffle_digit_positions:
        x = dataset.x[epoch_indices].view(-1, batch_size, sequence_length, 28, 28)
        x = x[:, :, torch.randperm(sequence_length)]
        x = x.reshape(-1, batch_size, sequence_length//2, 2, 28, 28).rot90(k=3, dims=(3,4))
        x = x.reshape(-1, batch_size, 280*28).transpose(1,2)
    else:
        x = dataset.x[epoch_indices].view(-1, batch_size, sequence_length//2, 2, 28, 28).rot90(k=3, dims=(3,4))
        x = x.reshape(-1, batch_size, 280*28).transpose(1,2)
    fixation_order_x = torch.ones((max_sequences, sequence_length, batch_size))
    fixation_order_y = torch.ones((max_sequences, sequence_length, batch_size))
    fixation_order_x[:, 0, :] = torch.randint(low=0, high=sequence_length//2, size=(max_sequences, batch_size))
    fixation_order_y[:, 0, :] = torch.randint(low=0, high=2, size=(max_sequences, batch_size))
    # # random vector with either 0 (vertical step) or -1 or 1
    steps = torch.randint(0, 3, size=(max_sequences, batch_size, sequence_length)) - 1
    steps_horizontal = torch.where(steps == 0, 1, 0)
    for i in range(1, sequence_length):
        fixation_order_x[:, i, :] = torch.clamp(fixation_order_x[:, i-1, :] + steps[:, :, i], min=0, max=4)
        fixation_order_y[:, i, :] = torch.remainder(fixation_order_y[:, i-1, :] + steps_horizontal[:, :, i], 2)
    fixations = torch.ones((max_sequences, sequence_length, 2, batch_size)) * 14
    fixations[:, :, 1, :] = fixations[:, :, 1, :] + fixation_order_x * 28
    fixations[:, :, 0, :] = fixations[:, :, 0, :] + fixation_order_y * 28
    fixations[:, :, 0, :] = fixations[:, :, 0, :] / 28 - 1
    fixations[:, :, 1, :] = fixations[:, :, 1, :] / 70 - 1
    fixations = fixations.reshape(-1, 10*2, batch_size)
    return x, fixations
